...
static uint8_t app_main_bredr_tx_power_table[POWER_LEVELS_CNT_MAX];
...
static void app_main_bt_read_tx_power_table_callback(wiced_bt_power_table_params_t *power_params);
...
APPLICATION_START( )
{
    ...
    WICED_BT_TRACE("******************************************\n");
    WICED_BT_TRACE("*  Headset WASS AAC Application Started  *\n");
    WICED_BT_TRACE("******************************************\n");
...
wiced_result_t app_main_post_init(void)
{
...
#ifdef FASTPAIR_ENABLE
    /* Stop LE advertisement by disabling GFPS provider if local device's role is
     * Secondary or not set yet. */
    if ((app_lrac_config_role_get() == WICED_BT_LRAC_ROLE_UNKNOWN) ||
        (app_lrac_config_role_get() == WICED_BT_LRAC_ROLE_SECONDARY))
    {
    ...
    {
        /* Decrease initial tx power on Secondary */
        /* It will assgin lower tx power table in the callback */
        for ( i = 0; i < POWER_LEVELS_CNT_MAX; i++)
        {
            wiced_bt_read_power_table_values(i, &app_main_bt_read_tx_power_table_callback);
        }
    }
    ...
    case APP_LRAC_CONNECTED:
        ...
        if (p_data->connected.status == WICED_BT_SUCCESS)
        {
            ...
            else
            {
                ...
                /* Set BR/EDR tx power table to original values on Secondary */
                APP_TRACE_DBG("Reset tx power table on Secondary\n");
                wiced_bt_dev_set_tx_power_table(POWER_LEVELS_MODE_BR, app_main_bredr_tx_power_table);
                wiced_bt_dev_set_tx_power_table(POWER_LEVELS_MODE_EDR, app_main_bredr_tx_power_table);
                wiced_bt_dev_force_update_tx_power_table();
            }
 ...
 *
 * Handle the case when the BT/EDR visibility has been changed.
 */
static void app_main_bt_visibility_change_handler(wiced_bool_t discoverable, wiced_bool_t connectable)
{
...
static void app_main_bt_read_tx_power_table_callback(wiced_bt_power_table_params_t *power_params)
{
    if (power_params->index < POWER_LEVELS_CNT_MAX)
    {
        app_main_bredr_tx_power_table[power_params->index] = power_params->power_value;
    }

    /* Last power table value returned */
    if (power_params->index == (POWER_LEVELS_CNT_MAX - 1))
    {
        uint8_t i = 0;
        uint8_t new_bredr_tx_power_table[POWER_LEVELS_CNT_MAX] = {0};

        WICED_BT_TRACE("app_main_bt_read_tx_power_table_callback:%d\n",
                  app_main_bredr_tx_power_table[POWER_LEVELS_CNT_MAX/2]);

        for (i = 0; i < POWER_LEVELS_CNT_MAX; i++)
        {
            if ( i < (POWER_LEVELS_CNT_MAX/2))
            {
                /* Set up lower power for index 0 to POWER_LEVELS_CNT_MAX/2 */
                new_bredr_tx_power_table[i] = app_main_bredr_tx_power_table[POWER_LEVELS_CNT_MAX/2];
            }
            else
            {
                new_bredr_tx_power_table[i] = app_main_bredr_tx_power_table[i];
            }
        }

        /* Assign new BR/EDR tx power table with lower power */
        wiced_bt_dev_set_tx_power_table(POWER_LEVELS_MODE_BR, new_bredr_tx_power_table);
        wiced_bt_dev_set_tx_power_table(POWER_LEVELS_MODE_EDR, new_bredr_tx_power_table);
        wiced_bt_dev_force_update_tx_power_table();
    }
}
